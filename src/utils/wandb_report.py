...
=====================
Experiment helpers for DA6401 Assignment-1 sections 2.1 – 2.10.

Every plot is:
  • Saved locally to  <save_dir>/plots/<section>_<n>.png
  • Logged to W&B as  wandb.Image under a descriptive key

All functions accept a wandb_run parameter.
train.py creates ONE persistent run per experiment and passes it here.
This ensures all plots appear under a single run and are visible in W&B reports.
"""
...
def init_wandb(args, name, group, extra_cfg=None, use_wandb=True):
    ...
    # finish any active run first to avoid reinit side-effects
    try:
...
def save_and_log(fig, filename: str, save_dir: str,
                 wandb_key: str, wandb_run=None, caption: str = ''):
    ...
    if wandb_run is not None and _WANDB_AVAILABLE:
        try:
            ...
            print(f"  Logged to W&B  key='{wandb_key}'")
...
def _reopen_run(wandb_run):
    """
    After per_run loops, reinit=True may have invalidated wandb_run.
    This resumes the run so it can still accept log() calls.
    Returns the same run object if still active, or reopens it.
    """
    if wandb_run is None or not _WANDB_AVAILABLE:
        return wandb_run
    try:
        # If run is still active just return it
...
def log_5_samples_from_each_class(X, y, image_shape=(28, 28),
    ...
    save_and_log(fig, '2.1_samples_grid.png', save_dir,
                 '2.1/samples_grid', wandb_run, caption='5 samples per class')
...
def run_sweep(args, CONFIG, x_train, y_train, NeuralNetwork=None):
    ...
    def sweep_train():
        ...
        run.name = (f"{cfg.optimizer}_lr{cfg.learning_rate}"
                    f"_{cfg.activation}_wd{cfg.weight_decay}_b{cfg.batch_size}")
        np.random.seed(42)
        model = NeuralNetwork(CONFIG, sweep_args)
        model.train(x_train, y_train,
                    epochs=sweep_args.epochs, batch_size=sweep_args.batch_size,
                    save_dir=sweep_args.save_dir, wandb_run=run)
        run.finish()
...
def optimizer_showdown(args, config, x_train, y_train,
    ...
    for opt in OPTS:
        ...
        model.train(x_train, y_train,
                    ...
                    save_dir=run_args.save_dir, wandb_run=per_run)
    ...
    save_and_log(fig, '2.3_optimizer_showdown_summary.png', args.save_dir,
                 ...
                 caption='Best val F1 per optimizer')
    plt.close(fig)
...
def vanishing_grad_analysis(args, config, x_train, y_train,
    ...
    # summary[depth][act] = list of gradient norms per layer
    ...
    for depth_name, neurons in CONFIGS.items():
        for act in ACTS:
            ...
            use_wb  = not getattr(args, 'no_wandb', True)
            per_run = init_wandb(run_args, f'vanishing_{label}',
    ...
    # ── Plot: one subplot per depth config ────────────────────────────────────
    ...
    fig.suptitle('2.4 - Vanishing Gradient: Gradient Norms per Layer (Sigmoid vs ReLU)',
                 fontsize=12)
    plt.tight_layout()
    ...
    save_and_log(fig, '2.4_vanishing_gradient_summary.png', args.save_dir,
                 ...
                 caption='Gradient norms per layer — shallow / medium / deep')
    plt.close(fig)
...
def dead_neuron_investigation(args, config, x_train, y_train,
    ...
    for cfg_override in configs_to_run:
        ...
        if val_accs:
            ...
            print(f"  Plateaued after epoch {plateau_epoch} / {len(val_accs)}")
            if per_run is not None and _WANDB_AVAILABLE:
                try:
                    per_run.log({'plateau_epoch': plateau_epoch,
                                 'best_val_acc' : best_val_acc})
                except Exception:
                    pass

        # ── Dead neuron fraction + activation stats per layer ──────────────
        dead_fracs    = []
        grad_summaries = []
        act_distributions = []
        ...
        for li, layer in enumerate(model.layers[:-1]):
            ...
            # Dead neurons (only meaningful for ReLU)
            ...
            if gf is not None:
                mean_grad = float(gf.mean())
                grad_summaries.append(mean_grad)
                print(f"  Layer {li} mean grad magnitude: {mean_grad:.6f}")
            else:
                grad_summaries.append(0.0)

            # Log per layer to wandb
            for run in [per_run, wandb_run]:
                if run is not None and _WANDB_AVAILABLE:
                    try:
                        run.log({
                            f'2.5/dead_{tag}/layer_{li}_dead_frac' : frac,
                            f'2.5/dead_{tag}/layer_{li}_mean_grad' : grad_summaries[-1],
    ...
    # ── Plots ─────────────────────────────────────────────────────────────────
    fig_bar   = plot_dead_bar(results)
    fig_dist  = plot_dead_dist(results, probe_X)
    fig_grad  = plot_grad_comparison(results)
    fig_val   = plot_val_accuracy(results)

    wandb_run = _reopen_run(wandb_run)
    save_and_log(fig_bar,  '2.5_dead_neuron_bar.png',    args.save_dir,
                 '2.5/dead_neuron_bar',       wandb_run,
                 caption='Dead neuron fraction per layer - ReLU vs Tanh vs Sigmoid')
    save_and_log(fig_dist, '2.5_activation_dist.png',    args.save_dir,
                 '2.5/activation_distribution', wandb_run,
                 caption='Activation distribution - ReLU vs Tanh vs Sigmoid')
    save_and_log(fig_grad, '2.5_gradient_flow.png',      args.save_dir,
                 '2.5/gradient_flow',         wandb_run,
                 caption='Mean gradient magnitude per layer - ReLU vs Tanh vs Sigmoid')
    save_and_log(fig_val,  '2.5_val_accuracy.png',       args.save_dir,
                 '2.5/val_accuracy',          wandb_run,
                 caption='Validation accuracy curves - plateau check')

    for fig in [fig_bar, fig_dist, fig_grad, fig_val]:
        plt.close(fig)
...
def loss_function_comparison(args, config, x_train, y_train,
    ...
    for loss_type in loss_types:
        ...
        per_run = init_wandb(run_args, f'loss_{loss_type}', '2.6_loss_comparison',
                             extra_cfg={'experiment': '2.6_loss_comparison',
                                        'loss_type': loss_type},
                             use_wandb=use_wb)
...
# 2.7
def global_performance_overlay(run_records: list, save_dir: str = '.', wandb_run=None):
    if not run_records:
    ...
    print(f"\n{'─'*60}")
    print(f"  2.7 Global Performance Overlay")
    print(f"{'─'*60}")
    print(f"  Total runs      : {len(run_records)}")
    print(f"  Mean gap        : {gap.mean():.4f}")
    print(f"  Max gap         : {gap.max():.4f}  ({names[np.argmax(gap)]})")
    print(f"  Best test acc   : {test_accs.max():.4f}  ({names[np.argmax(test_accs)]})")
    print(f"  Overfit runs    : {len(overfit_runs)} / {len(run_records)}  "
          f"(gap > {overfit_threshold*100:.0f}%)")
    if overfit_runs:
        print(f"\n  Top overfit runs:")
        for name, tr, te, g in sorted(overfit_runs, key=lambda x: -x[3])[:10]:
            print(f"    {name:40s}  train={tr:.4f}  test={te:.4f}  gap={g:.4f}")
    print(f"{'─'*60}\n")

    # ── Plot ──────────────────────────────────────────────────────────────────
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))

    # ── Left: scatter train vs test ───────────────────────────────────────────
    ax = axes[0]
    sc = ax.scatter(train_accs, test_accs, c=gap, cmap='RdYlGn_r',
                    s=60, alpha=0.75, edgecolors='k', linewidths=0.4,
                    vmin=0, vmax=max(gap.max(), overfit_threshold))
    ...
    ax.axhspan(lim[0], lim[1], alpha=0)   # invisible, just for layout

    # annotate top-5 overfit runs
    ...
    # ── Right: gap distribution histogram ────────────────────────────────────
    ax2 = axes[1]
    ax2.hist(gap, bins=20, color='#e74c3c', alpha=0.8,
             edgecolor='k', linewidth=0.5)
    ax2.axvline(0, color='k', linestyle='--', linewidth=1.2,
                label='No gap')
    ax2.axvline(overfit_threshold, color='darkred', linestyle='--',
                linewidth=1.2, label=f'Overfit threshold ({overfit_threshold})')
    ...
    save_and_log(fig, '2.7_global_performance_overlay.png', save_dir,
                 ...
                 caption='Train vs Test accuracy + gap distribution — every sweep run')
    plt.close(fig)
...
def error_analysis(model, x_test, y_test, dataset_name='mnist',
    ...
    save_and_log(fig_cm, '2.8_confusion_matrix.png', save_dir,
                 ...
                 caption=f'Confusion matrix - {dataset_name} test set')
    plt.close(fig_cm)
    ...
    save_and_log(fig_creative, '2.8_creative_failure_analysis.png', save_dir,
                 ...
                 caption='Creative failure analysis')
...
def weight_init_symmetry(args, config, x_train, y_train,
    ...
    for init in inits:
        ...
        run_args = make_args(args, {
            ...
            'learning_rate'   : 0.01,
            'num_neurons'     : [128, 128, 64],
            'activation'      : 'tanh',        # fix: was sigmoid
            'weight_decay'    : 0.0,
            'epochs'          : 1,             # fix: explicit 1 epoch
        ...
        model = NeuralNetwork(config, run_args)
        model.grad_history_layer0 = []         # fix: reset before each run
    ...
    save_and_log(fig, '2.9_weight_init_symmetry.png', args.save_dir,
                 ...
                 caption='Gradient norms: Zeros vs Xavier - 5 neurons, 50 steps')
    plt.close(fig)
    return history
...
def fashion_mnist_transfer(args, config, x_train, y_train,
    ...
    BEST_3 = [
        {
            'name'         : 'Config1_rmsprop_ReLU_128x3',
            'optimizer'    : 'rmsprop',
            'activation'   : 'relu',
            'num_neurons'  : [128, 128, 64],
            'learning_rate': 0.001,
        ...
        {
            'name'         : 'Config2_rmsprop_ReLU_128_64_32',
            'optimizer'    : 'momentum',
            'activation'   : 'relu',
            'num_neurons'  : [128, 128, 64],
            'learning_rate': 0.001,
        ...
        {
            'name'         : 'Config3_rmsprop_Tanh_128_64_32',
            'optimizer'    : 'momentum',
            'activation'   : 'tanh',
            'num_neurons'  : [128, 128, 64],
            'learning_rate': 0.001,
    ...
    for cfg_def in BEST_3:
        ...
        per_run = init_wandb(run_args, f'fashion_{name}', '2.10_fashion_transfer',
                             extra_cfg={'experiment': '2.10_fashion_transfer',
                                        'config_name': name},
                             use_wandb=use_wb)
        model = NeuralNetwork(config, run_args)
        model.train(x_train, y_train,
    ...
    save_and_log(fig, '2.10_fashion_transfer_summary.png', args.save_dir,
                 ...
                 caption='Test accuracy & F1 for 3 configs on Fashion-MNIST')
    plt.close(fig)
    return results