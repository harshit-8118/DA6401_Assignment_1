...
from .plots_fig import (plot_dead_bar, plot_symmetry, plot_dead_dist,
                        plot_grad_comparison, plot_loss_comparison,
                        plot_val_accuracy, creative_failure_viz)
...
def init_wandb(args, name, group, extra_cfg=None, use_wandb=True):
    ...
    try:
        ...
        pass
    return wandb.init(
        project = getattr(args, 'wandb_project', 'DA6401_Assignment1v4'),
    ...
    try:
        if wandb_run._is_finished:
            wandb_run = wandb.init(
                id=wandb_run.id, project=wandb_run.project,
                entity=wandb_run.entity, resume='allow', reinit=True,
            )
    except Exception:
        pass
    return wandb_run
...
# 2.1
def log_5_samples_from_each_class(X, y, image_shape=(28, 28), save_dir='.', wandb_run=None):
    if y.ndim > 1:
    ...
    for i, cls in enumerate(classes):
        ...
        for j in range(5):
            ...
            if j < len(sel):
                ...
                if wandb_run is not None and _WANDB_AVAILABLE:
                    wandb_images.append(wandb.Image(img, caption=f'Class {cls} | sample {j+1}'))
...
def run_sweep(args, CONFIG, x_train, y_train, NeuralNetwork=None):
    ...
    sweep_config = {
        ...
        'parameters': {
            ...
            'num_neurons'  : {'values': [[64, 32], [64, 64], [128, 128],
    ...
    def sweep_train():
        ...
        sweep_args = argparse.Namespace(
            dataset=args.dataset, epochs=cfg.epochs, batch_size=cfg.batch_size,
            learning_rate=cfg.learning_rate, optimizer=cfg.optimizer, loss=cfg.loss,
            weight_decay=cfg.weight_decay, num_neurons=num_neurons,
            activation=cfg.activation, weight_init=cfg.weight_init,
            seed=42, save_dir='sweep_models', model_save_path=f'sweep_{run.id}.npy',
        )
        ...
        model.train(x_train, y_train, epochs=sweep_args.epochs,
                    batch_size=sweep_args.batch_size, save_dir=sweep_args.save_dir, wandb_run=run)
        run.finish()
...
# 2.3
def optimizer_showdown(args, config, x_train, y_train, NeuralNetwork=None, wandb_run=None):
    OPTS, summary = ['sgd', 'momentum', 'nag', 'rmsprop'], {}

    for opt in OPTS:
        print(f"\nOptimizer Showdown: {opt.upper()}")
        run_args = make_args(args, {'optimizer': opt, 'num_neurons': [128, 128, 128],
                                    'activation': 'relu', 'weight_decay': 0.0,
                                    'model_save_path': f'showdown_{opt}.npy'})
        use_wb  = not getattr(args, 'no_wandb', True)
        per_run = init_wandb(run_args, f'showdown_{opt}', '2.3_optimizer_showdown',
                             extra_cfg={'experiment': '2.3_optimizer_showdown'}, use_wandb=use_wb)
        model = NeuralNetwork(config, run_args)
        model.train(x_train, y_train, epochs=run_args.epochs,
                    batch_size=run_args.batch_size, save_dir=run_args.save_dir, wandb_run=per_run)
    ...
    for bar, v in zip(bars, vals):
        ax.text(bar.get_x() + bar.get_width() / 2,
                bar.get_height() + 0.005, f'{v:.3f}', ha='center', va='bottom', fontsize=8)
    ax.set_ylim(0, 1)
    ...
    save_and_log(fig, '2.3_optimizer_showdown_summary.png', args.save_dir,
                 '2.3/optimizer_showdown_summary', _reopen_run(wandb_run))
    plt.close(fig)


# 2.4
def vanishing_grad_analysis(args, config, x_train, y_train, NeuralNetwork=None, wandb_run=None):
    ACTS    = ['sigmoid', 'relu']
    CONFIGS = {'shallow': [128], 'medium': [128, 128, 128], 'deep': [128, 128, 128, 128, 128]}
    ...
    for depth_name, neurons in CONFIGS.items():
        for act in ACTS:
            ...
            run_args = make_args(args, {'activation': act, 'optimizer': 'rmsprop',
                                        'num_neurons': neurons,
                                        'model_save_path': f'vanishing_{label}.npy'})
            use_wb  = not getattr(args, 'no_wandb', True)
            per_run = init_wandb(run_args, f'vanishing_{label}', '2.4_vanishing_gradient_analysis',
                                 extra_cfg={'experiment': '2.4_vanishing_gradient_analysis',
                                            'depth': depth_name}, use_wandb=use_wb)
            model = NeuralNetwork(config, run_args)
            model.train(x_train, y_train, epochs=run_args.epochs,
                        batch_size=run_args.batch_size, save_dir=run_args.save_dir, wandb_run=per_run)
    ...
    for ax, (depth_name, neurons) in zip(axes, CONFIGS.items()):
        ...
        for i, act in enumerate(ACTS):
            ...
            ax.bar(x_pos + i * width, norms, width, color=palette[act],
                   alpha=0.85, label=act, edgecolor='k', linewidth=0.6)
        ax.set_xticks(x_pos + width / 2)
        ax.set_xticklabels([f'L{i}' for i in range(n_layers)])
        ax.set_title(f'{depth_name.capitalize()} ({len(neurons)} hidden)')
        ax.set_ylabel('Gradient Norm')
        ax.legend()
    fig.suptitle('2.4 - Vanishing Gradient: Gradient Norms per Layer (Sigmoid vs ReLU)', fontsize=12)
    plt.tight_layout()
    save_and_log(fig, '2.4_vanishing_gradient_summary.png', args.save_dir,
                 '2.4/vanishing_gradient_summary', _reopen_run(wandb_run))
    plt.close(fig)


# 2.5
def dead_neuron_investigation(args, config, x_train, y_train, NeuralNetwork=None, wandb_run=None):
    ...
    results = {}
    (_, _), (x_val, y_val) = train_val_split(x_train, y_train,
                                              val_fraction=config['val_split'], seed=args.seed)
    probe_X = x_val[:500]

    for cfg_override in configs_to_run:
        tag = cfg_override.pop('tag')
        run_args = make_args(args, {**cfg_override, 'optimizer': 'sgd',
                                    'num_neurons': [128, 128, 128], 'weight_decay': 0.0,
                                    'model_save_path': f'dead_{tag}.npy'})
        print(f"\nDead Neuron Investigation - {tag}")
        use_wb  = not getattr(args, 'no_wandb', True)
        per_run = init_wandb(run_args, f'dead_{tag}', '2.5_dead_neuron',
                             extra_cfg={'experiment': '2.5_dead_neuron', 'tag': tag}, use_wandb=use_wb)
        model   = NeuralNetwork(config, run_args)
        history = model.train(x_train, y_train, epochs=run_args.epochs,
                              batch_size=run_args.batch_size, save_dir=run_args.save_dir, wandb_run=per_run)
        ...
        if val_accs:
            ...
            if per_run is not None and _WANDB_AVAILABLE:
                try:
                    per_run.log({'plateau_epoch': plateau_epoch, 'best_val_acc': best_val_acc})
                except Exception:
                    pass
        ...
        for li, layer in enumerate(model.layers[:-1]):
            ...
            print(f"  Layer {li}: {frac*100:.1f}% dead ({int(dead_mask.sum())}/{len(dead_mask)})")

            act_distributions.append({'mean': float(A.mean()), 'std': float(A.std()),
                                       'percent_zero': float((A == 0).mean() * 100), 'raw': A.copy()})
            ...
            for run in [per_run, wandb_run]:
                if run is not None and _WANDB_AVAILABLE:
                    try:
                        run.log({f'2.5/dead_{tag}/layer_{li}_dead_frac': frac,
                                 f'2.5/dead_{tag}/layer_{li}_mean_grad': mean_grad,
                                 f'2.5/dead_{tag}/layer_{li}_mean_act': act_distributions[-1]['mean']})
                    except Exception:
                        pass
            current_input = A

        results[tag] = {'model': model, 'dead_fracs': dead_fracs, 'grad_summaries': grad_summaries,
                        'act_distributions': act_distributions, 'val_accs': val_accs, 'run_args': run_args}
        _finish(per_run)

    wandb_run = _reopen_run(wandb_run)
    for fig, fname, key, cap in [
        (plot_dead_bar(results),          '2.5_dead_neuron_bar.png',  '2.5/dead_neuron_bar',        'Dead neuron fraction per layer'),
        (plot_dead_dist(results, probe_X),'2.5_activation_dist.png',  '2.5/activation_distribution','Activation distribution'),
        (plot_grad_comparison(results),   '2.5_gradient_flow.png',    '2.5/gradient_flow',          'Mean gradient magnitude per layer'),
        (plot_val_accuracy(results),      '2.5_val_accuracy.png',     '2.5/val_accuracy',           'Validation accuracy curves'),
    ]:
...
# 2.6
def loss_function_comparison(args, config, x_train, y_train, NeuralNetwork=None, wandb_run=None):
    loss_types, history = ['cross_entropy', 'mse'], {}

    for loss_type in loss_types:
        print(f"\nLoss Comparison - {loss_type.upper()}")
        run_args = make_args(args, {'loss': loss_type, 'optimizer': 'rmsprop',
                                    'num_neurons': [128, 128, 64], 'activation': 'relu',
                                    'weight_decay': 0.0, 'learning_rate': 0.001 * 0.1,
                                    'model_save_path': f'loss_{loss_type}.npy'})
        use_wb  = not getattr(args, 'no_wandb', True)
        per_run = init_wandb(run_args, f'loss_{loss_type}', '2.6_loss_comparison',
        ...
        def eval_hook(self_ref, X, y_oh, split_name='val'):
            ...
            if split_name == 'train':
                train_losses.append(metrics['loss']); train_accs.append(metrics['accuracy'])
            elif split_name == 'val':
                val_losses.append(metrics['loss']); val_accs.append(metrics['accuracy'])
            return metrics

        model.evaluate = types.MethodType(eval_hook, model)
        model.train(x_train, y_train, epochs=run_args.epochs,
                    batch_size=run_args.batch_size, save_dir=run_args.save_dir, wandb_run=per_run)
        history[loss_type] = {'train_loss': train_losses, 'val_loss': val_losses,
                               'train_acc': train_accs, 'val_acc': val_accs}
        _finish(per_run)

    fig = plot_loss_comparison(history)
    save_and_log(fig, '2.6_loss_comparison.png', args.save_dir, '2.6/loss_comparison',
                 _reopen_run(wandb_run), caption='Loss & accuracy convergence: CE vs MSE')
    plt.close(fig)
    return history
...
def global_performance_overlay(run_records, save_dir='.', wandb_run=None):
    ...
    overfit_runs      = [(names[i], train_accs[i], test_accs[i], gap[i])
                         for i in range(len(names)) if overfit_mask[i]]

    print(f"\n2.7 Global Performance Overlay")
    print(f"  Total runs: {len(run_records)}  |  Mean gap: {gap.mean():.4f}  |  "
          f"Max gap: {gap.max():.4f} ({names[np.argmax(gap)]})  |  "
          f"Best test acc: {test_accs.max():.4f} ({names[np.argmax(test_accs)]})")
    print(f"  Overfit runs: {len(overfit_runs)}/{len(run_records)} (gap > {overfit_threshold*100:.0f}%)")

    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    ax = axes[0]
    sc = ax.scatter(train_accs, test_accs, c=gap, cmap='RdYlGn_r', s=60, alpha=0.75,
                    edgecolors='k', linewidths=0.4, vmin=0, vmax=max(gap.max(), overfit_threshold))
    ...
    for i in np.argsort(gap)[-5:]:
        ax.annotate(names[i], (train_accs[i], test_accs[i]), fontsize=6, ha='right', color='darkred')
    ax.set_xlim(lim); ax.set_ylim(lim)
    ax.set_xlabel('Train Accuracy'); ax.set_ylabel('Test / Val Accuracy')
    ...
    ax2.axvline(overfit_threshold, color='darkred', linestyle='--', linewidth=1.2,
                label=f'Overfit threshold ({overfit_threshold})')
    ax2.set_xlabel('Train - Test Gap'); ax2.set_ylabel('Number of Runs')
    ax2.set_title('Gap Distribution'); ax2.legend(fontsize=8)
...
def global_performance_overlay_from_wandb(args, wandb_run=None, api_key=None, project=None):
    if not _WANDB_AVAILABLE:
        print("wandb not available - cannot pull sweep history.")
        return
    api     = wandb.Api(api_key=api_key)
    project = project or getattr(args, 'wandb_project', 'DA6401_Assignment1v4')
    entity  = getattr(args, 'wandb_entity', None)
    ...
    for r in api.runs(path):
        ...
        if ta is not None and va is not None:
            records.append({'name': r.name, 'train_acc': float(ta), 'test_acc': float(va)})
...
def error_analysis(model, x_test, y_test, dataset_name='mnist',
    ...
    plt.tight_layout()
    save_and_log(fig_cm, '2.8_confusion_matrix.png', save_dir, '2.8/confusion_matrix', wandb_run)
    plt.close(fig_cm)

    fig_creative = creative_failure_viz(x_test, y_true_lbl, y_pred_lbl, probs, confidences, cm, class_names)
    save_and_log(fig_creative, '2.8_creative_failure_analysis.png', save_dir,
                 '2.8/creative_failure_analysis', wandb_run)
    plt.close(fig_creative)


# 2.9
def weight_init_symmetry(args, config, x_train, y_train, NeuralNetwork=None, wandb_run=None,
                          n_neurons_to_track=5, track_grad_steps=50):
    inits, history = ['zeros', 'xavier'], {}

    for init in inits:
        print(f"\nWeight Init Symmetry - {init.upper()}")
        run_args = make_args(args, {'weight_init': init, 'optimizer': 'sgd', 'learning_rate': 0.01,
                                    'num_neurons': [128, 128, 64], 'activation': 'tanh',
                                    'weight_decay': 0.0, 'epochs': 1,
                                    'model_save_path': f'sym_{init}.npy'})
        use_wb  = not getattr(args, 'no_wandb', True)
        per_run = init_wandb(run_args, f'sym_{init}', '2.9_weight_init_symmetry',
                             extra_cfg={'experiment': '2.9_symmetry', 'init': init}, use_wandb=use_wb)
        model = NeuralNetwork(config, run_args)
        model.grad_history_layer0 = []
        model.train(x_train, y_train, epochs=run_args.epochs, batch_size=run_args.batch_size,
                    save_dir=run_args.save_dir, wandb_run=per_run, track_grad_steps=track_grad_steps)
    ...
    save_and_log(fig, '2.9_weight_init_symmetry.png', args.save_dir,
                 '2.9/weight_init_symmetry', _reopen_run(wandb_run))
    plt.close(fig)
    return history


# 2.10
def fashion_mnist_transfer(args, config, x_train, y_train,
                            x_test, y_test, NeuralNetwork=None, wandb_run=None):
    BEST_3 = [
        {'name': 'Config1_rmsprop_ReLU_128x3',   'optimizer': 'rmsprop', 'activation': 'relu',
         'num_neurons': [128, 128, 64], 'learning_rate': 0.001, 'weight_decay': 0.0,
         'batch_size': 64,  'epochs': getattr(args, 'epochs', 30)},
        {'name': 'Config2_rmsprop_ReLU_128_64_32','optimizer': 'momentum','activation': 'relu',
         'num_neurons': [128, 128, 64], 'learning_rate': 0.001, 'weight_decay': 0.0,
         'batch_size': 64,  'epochs': getattr(args, 'epochs', 30)},
        {'name': 'Config3_rmsprop_Tanh_128_64_32','optimizer': 'momentum','activation': 'tanh',
         'num_neurons': [128, 128, 64], 'learning_rate': 0.001, 'weight_decay': 0.0,
         'batch_size': 32,  'epochs': getattr(args, 'epochs', 30)},
    ]
    results = []
    ...
    for cfg_def in BEST_3:
        ...
        run_args = make_args(args, {**cfg_def, 'dataset': 'fashion_mnist',
                                    'loss': 'cross_entropy', 'weight_init': 'xavier',
                                    'config_path': f'fashion_{name}_config.json',
                                    'model_save_path': f'fashion_{name}.npy'})
        use_wb  = not getattr(args, 'no_wandb', True)
        per_run = init_wandb(run_args, f'fashion_{name}', '2.10_fashion_transfer',
        ...
        if per_run is not None and _WANDB_AVAILABLE:
            try:
                per_run.log({'test/accuracy': test_m['accuracy'],
                             'test/f1': test_m['f1'], 'test/loss': test_m['loss']})
            except Exception:
                pass
        _finish(per_run)
        ...
        if wandb_run is not None and _WANDB_AVAILABLE:
            try:
                wandb_run.log({f'2.10/test_acc/{name}': test_m['accuracy'],
        ...
        results.append({'name': name, 'test_acc': test_m['accuracy'],
                        'test_f1': test_m['f1'], **cfg_override})
    ...
    for ax, metric, ylabel in zip(axes, ['test_acc', 'test_f1'], ['Test Accuracy', 'Test F1 (macro)']):
        vals = [r[metric] for r in results]
        bars = ax.bar(short, vals, color=palette, alpha=0.85, edgecolor='k', linewidth=0.7)
        for bar, v in zip(bars, vals):
            ax.text(bar.get_x() + bar.get_width() / 2,
                    bar.get_height() + 0.005, f'{v:.3f}', ha='center', va='bottom', fontsize=9)
    ...
    save_and_log(fig, '2.10_fashion_transfer_summary.png', args.save_dir,
                 '2.10/fashion_transfer_summary', _reopen_run(wandb_run))
    plt.close(fig)
    return results