...
def run_sweep(args, CONFIG, x_train, y_train, x_test, y_test,
    ...
    sweep_config = {
        ...
        'parameters': {
            ...
            'hidden_size': {'values': [[64, 64], [128, 128], [128, 128, 64], [128, 128, 128]]},
...
def vanishing_grad_analysis(args, CONFIG, x_train, y_train,
    ...
    for depth_name, neurons in CONFIGS.items():
        for act in ACTS:
            ...
            sub = _begin_sub_run(run_args, f'vanishing_{label}', '2.4_vanishing_gradient_analysis', {'experiment': '2.4', 'depth': depth_name})
            model = NeuralNetwork(run_args)
            model.train(x_train, y_train,
                        epochs=run_args.epochs, batch_size=run_args.batch_size,
                        save_dir=run_args.save_dir, wandb_run=sub)
...
def dead_neuron_investigation(args, CONFIG, x_train, y_train,
    ...
    for fig, fname, key, cap in [
        ...
        (plot_grad_comparison(results),'2.5_gradient_flow.png', '2.5/gradient_flow', 'Gradient flow per layer'),
...
def global_performance_overlay(run_records, save_dir='.', wandb_run=None):
    ...
    plt.tight_layout()
    save_and_log(fig, '2.7_global_performance_overlay.png', save_dir, '2.7/global_performance_overlay', wandb_run)
    plt.close(fig)
...
def weight_init_symmetry(args, CONFIG, x_train, y_train, NeuralNetwork=None, wandb_run=None, n_neurons_to_track=5, track_grad_steps=50):
    inits = ['zeros', 'xavier']
    history = {}
    ...
    for init in inits:
        ...
        sub = _begin_sub_run(run_args, f'sym_{init}', '2.9_weight_init_symmetry', {'experiment': '2.9_symmetry', 'init': init})
        model = NeuralNetwork(run_args)
        model.grad_history_layer0 = []
        model.train(x_train, y_train, epochs=run_args.epochs, batch_size=run_args.batch_size, save_dir=run_args.save_dir, wandb_run=sub, track_grad_steps=track_grad_steps)

        grad_arr = np.array(model.grad_history_layer0)
        if grad_arr.ndim < 2 or len(grad_arr) == 0:
            is_finish(sub); continue
...
def fashion_mnist_transfer(args, CONFIG, x_train, y_train, x_test, y_test,
    ...
    for cfg_def in BEST_3:
        ...
        sub = _begin_sub_run(run_args, f'fashion_{name}', '2.10_fashion_transfer', {'experiment': '2.10', 'config_name': name})
        model = NeuralNetwork(run_args)
        model.train(x_train, y_train,
                    epochs=run_args.epochs, batch_size=run_args.batch_size,
                    save_dir=run_args.save_dir, wandb_run=sub)
