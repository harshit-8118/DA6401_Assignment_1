"""
Command-line argument definitions shared by train.py and inference.py.
"""
import argparse

ENTITY  = 'da25s003-indian-institute-of-technology-madras'
PROJECT = 'DA6401_Assignment1v6'
...
def parse_arguments():
    ...
    p.add_argument('-sz', '--hidden_size',   type=int,   nargs='+',
                   ...
                   help='Neurons per hidden layer. '
                        'If one value given it is broadcast to num_layers.')
    ...
    p.add_argument('--beta',   type=float, default=0.9,
                   help='Beta for momentum/RMSProp')
    ...
    # ── Misc ──────────────────────────────────────────────────────────────────
    p.add_argument('--experiment', type=str, default='train',
                   choices=['train', 'visual', 'sweep', 'optimizer', 'vanishing',
                            'dead', 'loss', 'overlay', 'error', 'symmetry',
                            'fashion', 'all'])
    ...
    p.add_argument('--val_fraction', type=float, default=None,
                   help='Validation set fraction (alias for val_split)')
    ...
    # Set val_fraction to val_split if not explicitly provided
